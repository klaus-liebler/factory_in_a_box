...
// Projektspezifische Kopie von libs/ST/usbx/common/core/inc/ux_user_sample.h (nur wirksam mit
// UX_INCLUDE_USER_DEFINE_FILE, s. CMakeLists.txt -- analog zu tx_user.h/nx_user.h). Bewusst als
// vollstaendige, dokumentierte Kopie der Vorlage gepflegt statt einer knappen Eigenfassung
// (gleiches Vorgehen wie bei tx_user.h/nx_user.h in diesem Projekt) -- nur die unten mit
// "-- FactoryControl:" markierten Zeilen weichen vom Sample ab. Werte anhand von ST's eigenem,
// tatsaechlich funktionierendem STM32H573I-DK-Referenzprojekt (Ux_Device_CDC_ECM) abgeglichen,
// nicht nur aus der allgemeinen Doku uebernommen.
...
/* -- FactoryControl: aktiv (nicht auskommentiert), an TX_TIMER_TICKS_PER_SECOND (Core/Inc/tx_user.h,
   Default 100) gekoppelt statt der USBX-Default-Annahme 1000 -- muss zum tatsaechlichen ThreadX-
   Tick dieses Projekts passen (analog zu ux_stm32_config.h). */
...
/* -- FactoryControl: 3 statt Default 1 -- ux_device_stack_class_register() wird pro Interface-
   Instanz aufgerufen (2x CDC-ACM: Debug + Modbus, 1x CDC-ECM), nicht pro Klassen-Typ (s.
   _ux_device_stack_class_register()'s class_index-Schleife ueber UX_MAX_SLAVE_CLASS_DRIVER
   Eintraege in libs/ST/usbx/common/core/src/ux_device_stack_class_register.c). */
#define UX_MAX_SLAVE_CLASS_DRIVER    3

/* Defined, this value represents the maximum number of interfaces in the device framework.  */
...
/* -- FactoryControl: bewusst NICHT gesetzt (USBX-Default 4096 bytes bleibt aktiv) -- entspricht
   ST's eigenem, tatsaechlich funktionierendem STM32H573I-DK-CDC-ECM-Referenzprojekt (dessen
   ux_user.h laesst diesen Wert ebenfalls unangetastet), statt wie das Sample hier vorbelegt
   willkuerlich auf 2048 zu reduzieren. */
...
/* -- FactoryControl: Host-Storage-Klasse ungenutzt (Device-only, UX_DEVICE_SIDE_ONLY unten) --
   Sample-Vorbelegung hier bewusst wieder auskommentiert. */
...
/* -- FactoryControl: Host-Controller-Simulator ungenutzt (Device-only) -- Sample-Vorbelegung
...
/* -- FactoryControl: Host-HID ungenutzt (Device-only) -- Sample-Vorbelegung wieder auskommentiert. */
...
/* -- FactoryControl: Host-Storage-Klasse ungenutzt (Device-only) -- Sample-Vorbelegung wieder
   auskommentiert. */
...
/* -- FactoryControl: aktiv -- CDC0/CDC1 (Debug/Modbus) teilen sich je eine Endpunkt-NUMMER
   zwischen IN und OUT (EP2/EP4, wie schon bei der TinyUSB-Fassung, s. usbd_device.c
   EP_CDC0_OUT=0x02/EP_CDC0_IN=0x82), um mit nur 8 physischen USB_DRD_FS-Endpunkten (EP0..EP7)
   auszukommen -- ohne dieses Define kann ux_dcd_stm32 IN/OUT derselben Nummer nicht getrennt
   fuehren (s. UX_DCD_STM32_ED_STRUCT::ux_dcd_stm32_ed_in in ux_dcd_stm32.h). Von ST's eigenem
   STM32H573I-DK-Referenzprojekt ebenso aktiv gesetzt. */
...
/* -- FactoryControl: aktiv -- dieses Geraet ist nie USB-Host (s. CMakeLists.txt-Kommentar zur
   Kuration von libs/ST/usbx: usbx_host_classes/usbx_host_controllers/ux_hcd_sim_host/
   ux_dcd_sim_slave sind gar nicht erst vendort). */
...
/* -- FactoryControl: bewusst NICHT aktiviert -- Standalone-Modus ersetzt die Klassen-eigenen
   ThreadX-Bulkin/-Bulkout-Threads durch von der Anwendung selbst zu pumpende _read_run()/
   _write_run()-Zustandsmaschinen (s. ux_device_class_cdc_acm_read_run.c) UND verlangt einen
   anderen ux_dcd_stm32-Aufrufpfad (_transfer_run() statt _transfer_request(), s. ux_dcd_stm32.h).
   Keines von ST's eigenen STM32H5-Referenzprojekten (Ux_Device_CDC_ECM, Ux_Device_HID_CDC_ACM)
   nutzt diesen Modus -- der Standard-Thread-Modus ist der einzige gegen echte Hardware verifizierte
   Pfad, den wir als Vorlage haben. */
...
/* -- FactoryControl: aktiv -- passt zum sonst im Projekt durchgaengigen XASSERT-Stil
   (ThreadX/NetX-Duo-Rueckgabewerte werden ueberall geprueft, s. z.B. net_setup.cpp); ST's
   eigenes Referenzprojekt laesst dies fuer Produktionsbuilds aus, wir sind hier aber explizit
   in der Erprobungsphase (neuer Branch, noch nicht auf Hardware verifiziert). */
