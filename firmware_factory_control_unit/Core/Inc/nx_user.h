...
/* NX_MAX_PHYSICAL_INTERFACES defines the number physical network interfaces
   ...
   in the system. */
/* USB-Composite-Device (s. usbd_device.c): zwei physische Interfaces -- Interface 0 ist der
   bestehende Ethernet-Port (nx_stm32_eth_driver, s. net_setup.cpp), Interface 1 die virtuelle
   CDC-ECM-NIC ueber USB (192.168.173.1, per nx_ip_interface_attach() in net_setup_create()
   angehaengt). Ohne diese Erhoehung schlaegt nx_ip_interface_attach() mit NX_NO_MORE_ENTRIES
   fehl (Default ist 1 physisches Interface). */
